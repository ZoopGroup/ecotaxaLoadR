testthat::test_that("build_master matches HEX tow with leading zeros", {
  ...
  testthat::expect_identical(names(result)[1], "sample_id")
  testthat::expect_identical(names(result)[match("net", names(result)) + 1], "fraction_ID")
  testthat::expect_true(all(is.na(result$fraction_ID)))
  testthat::expect_true(all(is.na(result$sample_id)))
  testthat::expect_equal(result$lat_dmm, "30\u00B000.000 N")
  testthat::expect_equal(result$long_dmm, "120\u00B000.000 W")
  testthat::expect_equal(result[["YYYYMMDD-HHMM_local"]], "20231231-1600")
  testthat::expect_true("Comments" %in% names(result))
  testthat::expect_true(all(is.na(result$Comments)))
})
...
testthat::test_that("build_master joins by tow and net when HEX net is available", {
  ...
  testthat::expect_identical(names(result_sorted)[1], "sample_id")
  testthat::expect_identical(names(result_sorted)[match("net", names(result_sorted)) + 1], "fraction_ID")
  testthat::expect_true(all(is.na(result_sorted$fraction_ID)))
  testthat::expect_true("YYYYMMDD-HHMM_local" %in% names(result_sorted))
  testthat::expect_true(all(result_sorted[["YYYYMMDD-HHMM_local"]] %in% c("20231231-1600", "20231231-1610")))
  testthat::expect_true("# boat splits" %in% names(result_sorted))
  testthat::expect_true("Comments" %in% names(result_sorted))
  testthat::expect_true(all(is.na(result_sorted[["# boat splits"]])))
  testthat::expect_true(all(is.na(result_sorted$Comments)))
})
